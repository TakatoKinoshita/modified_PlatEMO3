        ...
        function TableUpdate(obj,~,~,proindex)
            ...
            if ~isempty(obj.data)
                ...
                if strcmp(metric,'Number of runs')
                    ...
                    if ~isempty(styleLoc)
                        obj.app.table.addStyle(uistyle('FontWeight','bold', 'FontColor', 'r'),'cell',styleLoc);
                    end
                    ...
                    if obj.app.dropC(4).Value > 1
                        ...
                        for a = 1 : nA-1
                        	obj.app.table.Data{end,end-nA+a} = sprintf('%d/%d/%d',sum(sign1(:,a)),sum(sign3(:,a)),sum(sign2(:,a)));
                        end
                    else
                        obj.app.table.RowName = obj.app.table.RowName(1:nP);
                        obj.app.table.Data    = obj.app.table.Data(1:nP,:);
...
function table2excel(filename,sheetname,Data,styleLoc,nA)
    ...
    for i = 1 : size(styleLoc,1)
        Sheet.Range(getRange(styleLoc(i,1)+1,styleLoc(i,2)+1)).Font.Color = 255;
    end
    % Write the data
    Range.Value = Data;
...
function table2tex(filename,Data,styleLoc,nP,nA)
    ...
    mainData = regexprep(mainData,'e−','e-');
    for i = 1 : size(styleLoc,1)
